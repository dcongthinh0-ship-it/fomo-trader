...
当前每笔使用 `0.0004 ETH`（按 2026-09-15 查询价约为 1 美元），最多同时持有 3 个未平仓币；`POSITION_STUCK` 也占用持仓名额。持仓已满时到达的新信号立即永久跳过，不排队、不在以后空仓时回买；空仓后只买届时新到达的有效信号。ETH/USD 价格变化会让单笔美元价值随之变化。服务不会重新查询市值或流动性，也不会增加 K 线、风控评分、探针、止损、回撤、加仓或超时卖出。买入确认后以 receipt 的实际 token 数量建仓：

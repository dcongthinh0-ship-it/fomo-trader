...
- 退出状态机测试固定只有两种触发：`+40%` 全仓止盈，以及相对入场成本或上一有效全仓报价 `-90%` 的闪崩全仓卖出；`-80%` 不触发普通止损。闪崩 latch 在首次构建失败后仍保留，后续重取报价继续卖出，确认关闭后清除。
- SELL 的 `amountOutMinimum` 使用向下取整而非要求十进制结果恰好对应整数 base unit，覆盖普通 5% 与闪崩 50% 滑点产生的小数 wei，同时始终保持最小值非零。
